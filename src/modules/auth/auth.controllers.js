...
export const signUp = async (req, res, next) => {
    ...
    if(type === 'Doctor'){
        const emailEixsts = await DoctorModel.findOne({ email }).select('_id email')
        if (emailEixsts) {
        ...
        // confimation
        const token = tokenGeneration({ payload: { _id: newUser._id, email: newUser.email ,type : type ,role:newUser.role} })
        ...
        await newUser.save()
        return res.status(201).json({ message: "registration success , please confirm your email" })
    }
    ...
    // confimation
    const token = tokenGeneration({ payload: { _id: newUser._id, email: newUser.email ,role:newUser.role} })
...
export const confirmEmail = async (req, res, next) => {
    ...
    if(decode?.type){
        const doctorConfirmed = await DoctorModel.findOneAndUpdate({ _id: decode._id, isConfirmed: false }, {
            isConfirmed: true
        })
        if (!doctorConfirmed) {
...
export const login = async (req, res, next) => {
    ...
    if(type === 'Doctor'){
        ...
        if (!Doctor) {
            return next(new Error('please if enter a valid email or make sure that you confirm your email , Doctor', { cause: 400 }))
        }
        const match = pkg.compareSync(password, Doctor.password)
        if (!match) {
            return next(new Error('in-valid login information Doctor', { cause: 400 }))
        }
...
export const sendCode = async (req, res, next) => {
    ...
    const user = await userModel.findOne({ email, isConfirmed: true, role:systemRoles.USER  })

    if (! user  ) {
        // return next(new Error('please sign up fisrt', { cause: 400 }))
        const doctor = await DoctorModel.findOne({ email, isConfirmed: true, role:systemRoles.DOCTOR  })
        if (!doctor) {
            return next(new Error('Email not found. Please sign up first', { cause: 400 }));
        }
        // nanoid = hfj765765fhj
       const otp = nanoId()
    ...
    // const otpExpire = Date.now() + 3600000; expire in one hour
       const fromDateMoment = moment(new Date(otpExpire)).format('YYYY-MM-DD HH:mm:ms')
    ...
    // const message = `<p> OTP is ${forgetCode} </p>`
       ...
       const message = `<!DOCTYPE html>
        ...
        const saved = await DoctorModel.findOneAndUpdate({ email :doctor.email }, { otp,otpExpire }, { new: true })
        return res.status(200).json({ message: "OTP sent successfully", saved })
         ...
         // const message = `<p> OTP is ${forgetCode} </p>`
            ...
            const message = `<!DOCTYPE html>
             ...
             const saved = await userModel.findOneAndUpdate({ email :user.email }, { otp,otpExpire }, { new: true })
             return res.status(200).json({ message: "OTP sent successfully", saved })
