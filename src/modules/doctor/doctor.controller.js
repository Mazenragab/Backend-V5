...
export const uploadPhoto = async(req,res,next) => {
  ...
  if(!doctor.Image.public_id){
      ...
      folder:`${process.env.PROJECT_FOLDER}/Profile/Dr.${doctor.firstName} ${doctor.lastName}`
       })
       doctor.Image = {
          path:secure_url,
          public_id
  ...
  if (result.result !== 'ok' && result.result !== 'not found') {
    return next(new Error('Failed to delete user photo from Cloudinary', { cause: 400 }))
...
export const getAppointment = async(req,res,next) =>{
  ...
  const appointments = await appointmentModel.find({doctorId:doctor._id,isCanceled:false}).populate({path:"userId"})
